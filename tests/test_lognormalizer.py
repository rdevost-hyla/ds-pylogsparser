...
class Test(unittest.TestCase):
    ...
    def test_000_invalid_paths(self):
        ...
        self.assertRaises(Exception, bleh, ["/usr/bin/",])
    ...
    def test_002_deactivate_normalizer(self):
        ...
        to_deactivate = list(active_n.keys())[:2]
    ...
    def test_003_activate_normalizer(self):
        ...
        ln = LogNormalizer(self.normalizer_path)
        active_n = ln.get_active_normalizers()
        to_deactivate = list(active_n.keys())[0]
    ...
    def test_004_normalizer_uuid(self):
        ...
        self.assertTrue('uuid' in list(testlog.keys()))
    ...
    def test_005_normalizer_test_a_syslog_log(self):
        ...
        ln.lognormalize(testlog)
        self.assertTrue('uuid' in list(testlog.keys()))
        self.assertTrue('date' in list(testlog.keys()))
    ...
    def test_006_normalizer_test_a_syslog_log_with_syslog_deactivate(self):
        ...
        ln = LogNormalizer(self.normalizer_path)
        active_n = ln.get_active_normalizers()
        to_deactivate = [n for n in list(active_n.keys()) if n.find('syslog') >= 0]
        ...
        ln.lognormalize(testlog)
        self.assertTrue('uuid' in list(testlog.keys()))
        self.assertFalse('date' in list(testlog.keys()))
        self.assertFalse('program' in list(testlog.keys()))
    ...
    def test_007_normalizer_getsource(self):
        ...
        self.assertEqual(XMLfromstring(source).getroottree().getroot().get('name'), 'syslog')
